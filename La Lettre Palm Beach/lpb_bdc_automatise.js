...
function Choice(id, index, stackLetter, paymentMethod, starting_price_duration, renewal_term_length, starting_price,
     ...
     **/
     Choice.prototype.price = function () {
        ...
        if (this.isFreeMonth()) {
            if (this.starting_price_duration > 1){
                affichage =+ this.starting_price+ "&nbsp;€ le premier mois</span>";
            }else{
                affichage =+ this.starting_price + " 0&nbsp;€ les " + this.starting_price_duration + " premiers mois</span>";
            }
        }
        else if(this.isTrialMonth()){
            if (this.starting_price_duration > 1){
...
function customizeChoices(choices) {
    ...
    choices.forEach((choice, index) => {
        ...
        VANGUARD_LOCAL_CONFIG.choices[index].className = choices.length <= 4 || choices.length > 6 ? "my-1 col-md-6 col-12 mb-6" :
